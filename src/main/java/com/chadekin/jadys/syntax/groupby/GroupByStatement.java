...
/**
 * GroupByStatement
 * @param <G> The Post GroupBy statement
 */
public interface GroupByStatement<G> {
	
	public default G groupBy(String ...column_name){
		GroupByBuilderImpl builder = null;
		if(this instanceof JadysSqlQueryBuilder){
			JadysSqlQueryBuilder actualBuilder = ((JadysSqlQueryBuilder)this);
			builder = GroupByBuilderImpl.newGroupByStatement(actualBuilder);
			actualBuilder.setChild(builder);
			builder.apply(column_name);
		}
		return (G) builder;
	}
}
