 ...
 * AndOperator
 */
public interface AndOperator<T> extends SqlLogicalOperator {

	public default SqlFunctionProxy<ParameterOperator<T>> and(){
		executeLogicalOperator(JadysSqlOperation.AND);
		return (SqlFunctionProxy<ParameterOperator<T>>) this;
	}

	public default ParameterOperator<T> and(String column_name){
		executeLogicalOperator(JadysSqlOperation.AND, column_name);
		return (ParameterOperator<T>) this;
	}

	public default T and(SqlExpressionFactory expression){
		executeLogicalOperator(JadysSqlOperation.AND, expression);
		return (T) this;
	}
